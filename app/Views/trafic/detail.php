<div class="container">
    <div class="header">
        ...
        <p class="subtitle">Statistiques détaillées par bureau</p>
    </div>
        ...
        <table id="traficTable">
            <thead>
                <tr>
                    ...
                    <th><i class="fas fa-calendar-alt"></i>Date Opération</th>
                    <th><i class="fas fa-tasks"></i>Nom Opération</th>
                    <th><i class="fas fa-sort-numeric-up"></i>Nb Opérations</th>
                    <th><i class="fas fa-coins"></i>Montant Opération</th>
            ...
            <tbody id="tableBody">
                <?php if ($stats['total_lines'] > 0): ?>
                    ...
                    <tr data-code="<?= htmlspecialchars($row['CodeBureau']) ?>"
                        ...
                        data-date="<?= htmlspecialchars($row['DateOperation']) ?>"
                        data-operation="<?= htmlspecialchars($row['NomOperation']) ?>"
                        data-nbops="<?= $row['NombreReception'] ?>"
                        data-montant="<?= $row['MontantReception'] ?>"
                        data-droit="<?= $row['Droit'] ?>"
                        data-remb="<?= $row['REMB'] ?>"
                        data-nbremb="<?= $row['NBRREMB'] ?>">
                        ...
                        <td><div class="date-cell"><i class="fas fa-calendar"></i><?= htmlspecialchars($row['DateOperation']) ?></div></td>
                        <td><span class="badge badge-orange"><i class="fas fa-tasks" style="font-size:9px;"></i><?= htmlspecialchars($row['NomOperation']) ?></span></td>
                        <td><span class="badge badge-cyan"><i class="fas fa-sort-numeric-up" style="font-size:9px;"></i><?= number_format((float)$row['NombreReception'], 0, ',', ' ') ?></span></td>
                        <td><span class="badge badge-green"><i class="fas fa-coins" style="font-size:9px;"></i><?= number_format((float)$row['MontantReception'], 2, ',', ' ') ?> DA</span></td>
                        <td><span class="badge badge-pink"><i class="fas fa-gavel" style="font-size:9px;"></i><?= number_format((float)$row['Droit'], 2, ',', ' ') ?> DA</span></td>
                        <td><span class="badge badge-red"><i class="fas fa-undo" style="font-size:9px;"></i><?= number_format((float)$row['REMB'], 2, ',', ' ') ?> DA</span></td>
                        <td><span class="badge badge-cyan"><i class="fas fa-sort-numeric-up" style="font-size:9px;"></i><?= number_format((float)$row['NBRREMB'], 0, ',', ' ') ?></span></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="9">
                            <div class="empty-state">
                                ...
                                <p>Aucun trafic enregistré pour les bureaux de la wilaya 31.</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
    ...
    <div class="stats-bar">
        ...
        <div class="stat-item">
            <div class="stat-icon green"><i class="fas fa-sort-numeric-up"></i></div>
            <div class="stat-info">
...
<script>
    ...
    document.querySelectorAll('#tableBody tr').forEach(row => {
        if (row.cells.length === 9 && !row.querySelector('.empty-state')) {
            allRows.push({
    ...
    function performSearch() {
        ...
        if (selectedBureaus.length < uniqueBureaus.length) {
            filteredRows = filteredRows.filter(r => selectedBureaus.includes(r.nom));
        }
        if (selectedOperations.length < uniqueOperations.length) {
            filteredRows = filteredRows.filter(r => selectedOperations.includes(r.operation));
        }
        if (searchTerm !== '') {
            filteredRows = filteredRows.filter(r =>
                ...
                r.operation.toLowerCase().includes(searchTerm) ||
                r.nbOps.toString().includes(searchTerm) ||
                r.montant.toString().includes(searchTerm) ||
                r.droit.toString().includes(searchTerm) ||
                r.remb.toString().includes(searchTerm) ||
                r.nbRemb.toString().includes(searchTerm)
            );
        }
        ...
        if (filteredRows.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="9"><div class="empty-state"><i class="fas fa-search"></i><h3>Aucun résultat</h3><p>Modifiez vos filtres.</p></div></td></tr>';
            return;
        }

        let html = '';
        filteredRows.forEach(r => {
            let code = escapeHtml(r.code), nom = escapeHtml(r.nom), date = escapeHtml(r.date),
                op = escapeHtml(r.operation), nbOps = formatNumber(r.nbOps),
                montant = formatMontant(r.montant), droit = formatMontant(r.droit),
                remb = formatMontant(r.remb), nbRemb = formatNumber(r.nbRemb);

            if (searchTerm) {
                const regex = new RegExp('(' + escapeRegex(searchTerm) + ')', 'gi');
                code = code.replace(regex, '<mark>$1</mark>');
                nom = nom.replace(regex, '<mark>$1</mark>');
                date = date.replace(regex, '<mark>$1</mark>');
            ...
            html += '<tr>' +
                ...
                '<td><div class="date-cell"><i class="fas fa-calendar"></i>' + date + '</div></td>' +
    ...
    function getFilteredData() {
        ...
        if (searchTerm) {
            filtered = filtered.filter(r =>
                ...
                r.date.toLowerCase().includes(searchTerm) || r.operation.toLowerCase().includes(searchTerm) ||
                r.nbOps.toString().includes(searchTerm) || r.montant.toString().includes(searchTerm) ||
                r.droit.toString().includes(searchTerm) || r.remb.toString().includes(searchTerm) ||
                r.nbRemb.toString().includes(searchTerm)
            );
        }
        return filtered.map(r => ({
            'Code Bureau': r.code, 'Nom Bureau': r.nom, 'Date Opération': r.date,
            'Nom Opération': r.operation, 'Nb Opérations': r.nbOps,
            'Montant Opération (DA)': r.montant, 'Droit (DA)': r.droit,
            'Remboursement (DA)': r.remb, 'Nb Remboursements': r.nbRemb
        }));
    }
    ...
    function exportExcel() {
        ...
        ws['!cols'] = [{wch:15},{wch:25},{wch:15},{wch:30},{wch:15},{wch:18},{wch:15},{wch:18},{wch:15}];
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Trafic_Bureaux_Detail');
        XLSX.writeFile(wb, 'trafic_bureaux_detail.xlsx');
    }
    ...
    function exportPDF() {
        ...
        doc.text('Trafic Bureau Détail - Wilaya 31 (Oran)', 14, 16);
        ...
        const totalMontant = data.reduce((s,r) => s + r['Montant Opération (DA)'], 0);
        ...
        const body = data.map(r => [r['Code Bureau'], r['Nom Bureau'], r['Date Opération'], r['Nom Opération'],
            r['Nb Opérations'].toString(), r['Montant Opération (DA)'].toLocaleString('fr-FR')+' DA',
            r['Droit (DA)'].toLocaleString('fr-FR')+' DA', r['Remboursement (DA)'].toLocaleString('fr-FR')+' DA',
            r['Nb Remboursements'].toString()]);
        doc.autoTable({
            head: [['Code Bureau','Nom Bureau','Date','Opération','Nb Ops','Montant','Droit','Remboursement','Nb Remb']],
            body: body, startY: 47,
        ...
        doc.save('trafic_bureaux_detail.pdf');
    }

    function exportTXT() {
        const data = getFilteredData();
        if (!data.length) { alert('Aucune donnée à exporter.'); return; }
        let content = '='.repeat(120) + '\nTRAFIC BUREAU DÉTAIL - WILAYA 31 (ORAN)\n' + '='.repeat(120) + '\n';
        content += "Date d'export : " + new Date().toLocaleString('fr-FR') + '\n';
        content += 'Nombre de lignes : ' + data.length + '\n';
        content += '='.repeat(120) + '\n\n';
        content += 'Code Bureau | Nom Bureau | Date Opération | Nom Opération | Nb Opérations | Montant | Droit | Remboursement | Nb Remb\n';
        content += '-'.repeat(120) + '\n';
        data.forEach(r => {
            content += r['Code Bureau']+' | '+r['Nom Bureau']+' | '+r['Date Opération']+' | '+r['Nom Opération']+' | '+r['Nb Opérations']+' | '+r['Montant Opération (DA)'].toLocaleString('fr-FR')+' DA | '+r['Droit (DA)'].toLocaleString('fr-FR')+' DA | '+r['Remboursement (DA)'].toLocaleString('fr-FR')+' DA | '+r['Nb Remboursements']+'\n';
        });
        content += '\n'+'='.repeat(120)+'\nFin du rapport\n';
        const blob = new Blob([content], {type:'text/plain;charset=utf-8'});
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = 'trafic_bureaux_detail.txt';
